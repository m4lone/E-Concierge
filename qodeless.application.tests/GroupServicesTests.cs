...
namespace qodeless.application.tests
{
    ...
    public class GroupServicesTests
    {
        ...
        {
            // given (arrange)
            var groupId = It.IsAny<Guid>(); /*Função da biblioteca MOQ*/
            var group = new GroupViewModel() { Id = groupId, Name = "TESTE", Code = "1", AcceptanceCriteria = 51 };

            // when (act)
            var validationResult =  groupAppService.Add(group);
        ...
        public async Task GetGroupById_ShouldReturnFieldName()
        {
            ...
            // when (act)
            var groupsFromService =  groupAppService.GetById(entityId);
